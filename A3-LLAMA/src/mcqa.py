...
def parse_arguments():
    ...
    parser.add_argument('--input_file', type=str, default="data/test.jsonl",
                        help='Path to input JSONL file (train/valid/test)')
    parser.add_argument('--output_file', type=str, default="predictions/test.jsonl",
                        help='Path to output predictions JSONL file')

    # Model arguments
    # parser.add_argument('--model_path', type=str, required=True,
    #                     help='Path to the model or model name in HuggingFace hub')
    parser.add_argument('--model_path', type=str, default="models/Llama-3.2-1B-Instruct",
                        help='Path to the model or model name in HuggingFace hub')
    parser.add_argument('--device', type=str, default=None,
                        help='Device to use (cuda/cpu). If None, automatically detected')

    # Voting arguments
    parser.add_argument('--num_forward_passes', type=int, default=8,
                        help='Number of forward passes for majority voting')
    parser.add_argument('--min_probability_threshold', type=float, default=0.1,
                        help='Minimum probability threshold for a prediction to be considered')
...
def majority_vote(all_predictions, min_probability_threshold):
    ...
    for preds, rationale in all_predictions:
        # Only consider predictions above the threshold 选择超过阈值的key
