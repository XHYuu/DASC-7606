...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, time_embedding_dim):
        ...
        #     stride=2
        # )
        self.time_mlp = TimeMLP(embedding_dim=time_embedding_dim, hidden_dim=in_channels * 2 , out_dim=in_channels)
        # self.conv1 = nn.Sequential(*[ResidualBottleneck(in_channels, in_channels) for i in range(2)],
        #                            ResidualBottleneck(in_channels, out_channels),
        #                            ResidualBottleneck(out_channels, out_channels // 2))
        # self.conv1 = nn.Sequential(*[ResidualBottleneck(in_channels, in_channels) for i in range(3)],
        #                            ResidualBottleneck(in_channels, out_channels // 2))
